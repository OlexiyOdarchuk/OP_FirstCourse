...
int countBitsRecursive(unsigned int n)
{
    ...
    int result = 0; //кількість одиничних бітів
...
{
    int count = 0;         //кількість одиничних бітів
    unsigned int rest = n; //залишок з непогашеними бітами

    *iterations = 0; //обнулити лічильник ітерацій

    while (rest != 0) //повторювати, доки є одиниці
    {
...
bool parseUnsigned(const char *text, unsigned int *value)
{
    ...
    for (const char *p = text; *p != '\0'; ++p) //перебрати символи рядка
    {
        ...
        const unsigned int digit = (unsigned int)(*p - '0'); //обчислити значення цифри
...
int main(void)
{
    ...
    {
        const int scanned = scanf("%11s", token); //увести слово
    ...
    const int recResult = countBitsRecursive(n); //обчислити кількість рекурсивно
