...
{
    int root = 0; //кандидат на значення кореня
...
bool fillFromKeyboard(int a[], int n)
{
    ...
    {
        char prompt[32];                                  //запрошення для елемента
        snprintf(prompt, sizeof prompt, "  a[%d] = ", i); //сформувати запрошення
...
void siftDown(int a[], int root, int size)
{
    ...
    {
        int largest = root;             //індекс найбільшого з трьох елементів
        const int left = 2 * root + 1;  //індекс лівого нащадка
        const int right = 2 * root + 2; //індекс правого нащадка
...
void heapSort(int a[], int n, bool verbose)
{
    ...
    for (int root = n / 2 - 1; root >= 0; --root) //перебрати внутрішні вузли до кореня
    {
        siftDown(a, root, n); //просіяти вузол вниз
        if (verbose)          //якщо потрібні проміжні ітерації
        {
            printf("  просіяно вузол %d:", root); //вивести номер просіяного вузла
    ...
    for (int size = n - 1; size > 0; --size) //зменшувати розмір купи
    {
...
int blockSearch(const int a[], int n, int key, unsigned int *comparisons, int *blocks)
{
    ...
    *blocks = 0;      //обнулити лічильник блоків
    ...
    int step = integerSqrt(n); //довжина блоку
    ...
    int prev = 0;           //початок поточного блоку
    int current = step - 1; //межа поточного блоку
    ...
    for (int i = prev; i <= current; ++i) //перебрати елементи блоку
    {
        ++(*comparisons); //врахувати порівняння
...
int main(void)
{
    ...
    if (!readInt(prompt, &n, 1, MAX_SIZE)) //якщо кількість не введено
    {
    ...
    int choice = 0; //обраний спосіб створення масиву
    ...
    int mode = 0;                                        //обраний режим виведення
    if (!readInt("Оберіть режим (1..2): ", &mode, 1, 2)) //якщо режим не обрано
    {
        return 1; //завершити з кодом помилки
    }
    ...
    unsigned int searchComparisons = 0; //кількість порівнянь під час пошуку
    int blocks = 0;                     //кількість переглянутих блоків
