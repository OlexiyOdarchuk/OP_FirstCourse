...
void printArray(const char *title, const int a[], int n)
{
    ...
    for (int i = 0; i < n; ++i) //перебрати елементи масиву
    {
        const int length = snprintf(NULL, 0, "%d", a[i]) + 1; //довжина запису числа
    ...
    for (int i = 0; i < n; ++i) //перебрати індекси елементів
    {
        printf("%*d", width, i); //вивести індекс
    }
...
{
    int swaps = 0; //лічильник виконаних обмінів

    for (int i = 0; i + 1 < n; i += 2) //перебрати пари сусідніх елементів
    {
...
int main(void)
{
    ...
    const int swaps = swapEvenOdd(a, n); //обміняти елементи, отримати кількість обмінів
